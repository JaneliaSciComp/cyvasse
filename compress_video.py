...
def compress_video(input_video_path,
                   output_video_path):
    # Set the path to use for ffmpeg
    ffmpeg_executable_name = '/groups/svoboda/home/svobodalab/bin/ffmpeg'
    ...
    with tempfile.TemporaryDirectory(prefix=generalized_slash_tmp_path + "/") as scratch_folder_path:
        ...
        #return_code = os.system('ffmpeg -y -i "%s" "%s/frame-%%d.png"' % (input_video_path, frames_folder_path))
        command = [ ffmpeg_executable_name, '-y', '-i', input_video_path, os.path.join(frames_folder_path, 'frame-%d.png') ]
        (return_code, stdout_as_string, stderr_as_string) = dlct.system(command)
        if return_code != 0 :
            raise RuntimeError(
        ...
        command = [ ffmpeg_executable_name, '-y', '-framerate', '1', '-i', os.path.join(scratch_folder_path, 'frames', 'frame-%d.png'),
...
if __name__ == "__main__" :
    ...
    if len(sys.argv) >= 3:
        ...
        output_video_path = dlct.replace_extension(input_video_path, '.mp4')
